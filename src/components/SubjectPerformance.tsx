...
function SubjectDetailsModal({ subjectName, stats }: { subjectName: string, stats: any }) {
  ...
  const lastActivity = attempts && attempts.length > 0 && attempts[0].timestamp ? attempts[0].timestamp : null;
  const lastActivityFormatted = lastActivity && typeof lastActivity.toDate === 'function' ? format(lastActivity.toDate(), "dd 'de' MMMM 'de' yyyy", { locale: ptBR }) : null;

  const accuracy = calculatePercentage(stats.correct, stats.answered);
  const errors = stats.answered - stats.correct;
  const SubjectIcon = getSubjectIcon(subjectName);

  const recentAttempts = attempts || [];
  const recentCorrect = recentAttempts.reduce((acc, curr) => acc + (curr.isBatch && curr.batchCorrect !== undefined ? curr.batchCorrect : (curr.isCorrect ? 1 : 0)), 0);
  const recentTotal = recentAttempts.reduce((acc, curr) => acc + (curr.isBatch && curr.batchTotal !== undefined ? curr.batchTotal : 1), 0);
  const recentAccuracy = recentTotal > 0 ? calculatePercentage(recentCorrect, recentTotal) : 0;
      ...
      <div className="flex-1 overflow-y-auto p-8 space-y-10 bg-slate-50/30">
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <div className="p-6 rounded-[2rem] bg-white border border-slate-100 shadow-sm flex flex-col items-center text-center group hover:scale-[1.03] transition-all">
            <span className="text-xs font-black text-slate-400 uppercase tracking-widest mb-1">Resolvidas</span>
            <span className="text-3xl font-black text-slate-900">{stats.answered}</span>
          </div>
          <div className="p-6 rounded-[2rem] bg-emerald-50/50 border border-emerald-100/50 shadow-sm flex flex-col items-center text-center group hover:scale-[1.03] transition-all">
            <span className="text-xs font-black text-emerald-600/60 uppercase tracking-widest mb-1">Acertos</span>
            <span className="text-3xl font-black text-emerald-600">{stats.correct}</span>
          </div>
          <div className="p-6 rounded-[2rem] bg-rose-50/50 border border-rose-100/50 shadow-sm flex flex-col items-center text-center group hover:scale-[1.03] transition-all">
            <span className="text-xs font-black text-rose-600/60 uppercase tracking-widest mb-1">Erros</span>
            <span className="text-3xl font-black text-rose-600">{errors}</span>
          </div>
          <div className={cn(
            "p-6 rounded-[2rem] border shadow-md flex flex-col items-center text-center group hover:scale-[1.03] transition-all relative overflow-hidden",
            accuracy >= 80 ? 'bg-emerald-500 border-none' :
              accuracy >= 60 ? 'bg-accent border-none' :
                'bg-destructive border-none'
          )}>
            <div className="absolute inset-0 bg-black/10 mix-blend-overlay" />
            <span className="text-xs font-black text-white/80 uppercase tracking-widest mb-1 relative z-10">Precisão</span>
            <span className="text-3xl font-black text-white relative z-10">{accuracy.toFixed(0)}%</span>
          </div>
        </div>
          ...
          <div className="space-y-5">
            <div className="flex items-center justify-between px-2">
              <div className="flex items-center gap-2">
                ...
                <h4 className="font-black text-[11px] uppercase tracking-[0.2em] text-slate-950">Último Treino</h4>
              </div>
              <Badge variant="outline" className="bg-white border-slate-200 text-[9px] font-black">{activeDays.size} DIAS ATIVOS</Badge>
            </div>
            <div className="p-6 rounded-[2.5rem] border-2 border-white bg-white shadow-[0_20px_50px_rgba(0,0,0,0.05)] flex flex-col items-center justify-center relative overflow-hidden text-center min-h-[220px]">
              <div className="absolute inset-0 bg-gradient-to-tr from-accent/5 via-transparent to-transparent opacity-50 pointer-events-none" />
              ...
              {isLoading ? (
                ...
                <div className="relative z-10 w-full flex flex-col items-center">
                  <div className="flex w-full items-center justify-between px-2 mb-4">
                    <div className="flex items-center gap-2">
                      ...
                      <div className="text-left">
                        <span className="block text-[9px] font-black text-slate-400 uppercase tracking-widest">Taxa de Sucesso Recente</span>
                        <span className="block font-black text-slate-800 text-lg">{recentAccuracy.toFixed(0)}% <span className="text-xs text-slate-400 font-bold ml-1">({recentTotal} questões)</span></span>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="flex flex-col items-center pt-2">
                    <span className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] mb-1">Última vez que resolveu questões:</span>
                    <span className="text-xl font-black text-slate-900 drop-shadow-sm">{lastActivityFormatted}</span>
                  </div>
                </div>
              ) : (
                <div className="relative z-10 flex flex-col items-center">
                  ...
                  <span className="text-xl font-black text-slate-400">Sem histórico recente</span>
                </div>
              )}
            </div>
            <div className="flex items-center justify-center gap-3 p-4 rounded-2xl bg-slate-900 text-[10px] font-bold text-white uppercase tracking-widest border-b-4 border-slate-950 shadow-xl">
              ...
              <span>Mantenha o Ritmo Constante</span>
            </div>
          </div>
